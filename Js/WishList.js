...
// current_user = {
//     "id": "Us-1", "Fname": "mohamed", "Lname": "Kandeel",
...
function checkWhichPage() {
    if (!JSON.parse(localStorage.getItem('WishLists'))) {
        checkEmpty();
    } else if (JSON.parse(localStorage.getItem('WishLists')).length === 0) {
        checkEmpty();
    } else if (JSON.parse(localStorage.getItem('WishLists')).find(item => item.userid === current_user_Id) && JSON.parse(localStorage.getItem('WishLists')).find(item => item.userid === current_user_Id).wish_prod.length === 0) {
        checkEmpty();
    }
    else {
...
window.addEventListener('load', function () {
    ...
    // Show login modal if no current user
    if (isGuest) {
        const loginModal = new bootstrap.Modal(document.getElementById('loginAlertModal'));
        loginModal.show();
        return; // Stop loading wishlist data
    }
    ...
    add(productId) {
        ...
        for (let i = 0; i < this.wish_prod.length; i++) {
            if (this.wish_prod[i] == product) {
                showToast('Product already on your wishlist!', 'error');
                return;
            }
        }
...
function delett(event) {
    ...
    if (el && el.dataset.id) {
        user_wish.delete(Number(el.dataset.id));
    }
    //user_wish.delete(38);
